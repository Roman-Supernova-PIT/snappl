...
class lightcurve:
    ...
    def __init__(self, data, meta):
        ...
        for b in unique_bands:
            required_meta_cols.append(f"local_surface_brightness_{b}")
        ...
        for col, col_type in zip(required_data_cols, required_data_col_types):
            assert col in data_cols[:len(required_data_cols)+1], f"Missing required data column {col}" + \
            f" in the first {len(required_data_cols)} columns."
