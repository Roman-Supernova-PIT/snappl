...
def _parse_rdm_file( sourcepath=None, dest_base_path=None, dest_subdir=None, provid=None,
                     rellink=False, abslink=False, really_do=False ):
    ...
    if really_do and ( filepath != writepath ):
        if writepath.exists():
            ...
            if abslink:
                os.symlink( sourcepath, writepath )
            elif rellink:
                linkdest = sourcepath.relative_to( writepath.parent, walk_up=True )
                os.symlink( linkdest, writepath )
            else:
                # copy2 doesn't work inside singularity because of group problems,
                #   so punt on saving file timestamps and such
                # shutil.copy2( sourcepath, writepath )
                shutil.copyfile( sourcepath, writepath )
...
class RDM_L2image_loader:
    def __init__( self, provid=None, source_path=None, dest_path=None, dest_subdir=None,
                  regex_image=None, relative_symlink=False, absolute_symlink=False, really_do=False ):
        ...
        self.relative_symlink = relative_symlink
        self.absolute_symlink = absolute_symlink
    ...
    def omg( self, e ):
        SNLogger.debug( f"Got an error: {e}" )
    ...
    def __call__( self, loadevery=1000, nprocs=1, filelist=None ):
        ...
        do_parse_rdm_file = functools.partial( _parse_rdm_file,
                                               ...
                                               rellink=self.relative_symlink,
                                               abslink=self.absolute_symlink,
...
def main():
    parser = argparse.ArgumentParser( 'load_ou2024_l2images',
                                      description='Load roman datamodel image files from Rick Sims below a directory.',
                                      formatter_class=argparse.ArgumentDefaultsHelpFormatter )
    parser.add_argument( '-p', '--provid', required=True, help="Provenance id" )
    parser.add_argument( '-n', '--nprocs', type=int, default=8,
                         help="Number of processes to run at once [default: 8]" )
    parser.add_argument( '-s', '--sourcedir', default='/ricksims/output_images_2026-08_LBNL_mtg',
    ...
    parser.add_argument( '-r', '--regex-image', default=".*_L2\\.asdf$",
                         help="Regex that filename must match to be considerd an image file we want to load" )
    parser.add_argument( '-l', '--relative-link', default=False, action='store_true',
                         help=( "Don't copy files, instead put in relative symbolic links, "
                                "if you know what you're doing" ) )
    parser.add_argument( '-a', '--absolute-link', default=False, action='store_true',
                         help=( "Con't copy files, instead put in absolute symbolic links, "
                                "if you know what you're doing" ) )
    ...
    loader = RDM_L2image_loader( provid=args.provid,
                                 ...
                                 relative_symlink=args.relative_link,
                                 absolute_symlink=args.absolute_link,
                                 really_do=args.do )
