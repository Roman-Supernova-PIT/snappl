...
class Lager:
    """Holds the logging instance that we use throughout stuff.

    Normal use: get the logger object with Lager.get(), which is a
    stander logging logger object.  Or, just call Lager.debug,
    Lager.info, Lager.warning, or Lager.error, which will
    instantiate the singleton as necessary.

    The single instance is held inside a Lager singleton.  In
    principle, you could also make other Lager objects, but that's
    ...
    def instance( cls, midformat=None, datefmt=_default_datefmt, level=_default_log_level ):
        """Return the singleton instance of Lager."""
    ...
    def replace( cls, midformat=None, datefmt=None, level=None ):
        ...
        Subsequent calls to Lager.get(), .info(), etc. will now
    ...
    def __init__( self, midformat=None, datefmt=_default_datefmt,
        ...
        """Initialize a Lager object, and the logging.Logger object it holds.
        ...
        """
        Lager._ordinal += 1
        self._logger = logging.getLogger( f"SeeChange_{Lager._ordinal}" )
